...
A shape describes the keys of a record, in order. It *is* the record's type. All records with the same keys in the same order share exactly one shape.
...
There are multiple ways to construct a shape value.
